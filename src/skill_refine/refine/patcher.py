...
from skill_refine.core.models import BoundaryConfidence, PatchProposal, Skill
from skill_refine.core.sections import find_section
...
def assess_boundary_confidence(skill: Skill, section_name: str) -> BoundaryConfidence:
    ...
    HIGH: section has clear ## boundaries, content is non-empty
    MEDIUM: section exists but is very short or adjacent to unusual structure
    LOW: section not found, or body has no clear heading structure
    """
...
def apply_patch(skill: Skill, proposal: PatchProposal) -> str:
    ...
    # Reconstruct full file: replace the body portion of raw_content.
    # The body starts after the frontmatter in raw_content.
...
def _find_section_span(body: str, section_name: str) -> tuple[int, int] | None:
    ...
    Returns the byte offsets of the content area between the heading line
    and the next heading (or end of body). Returns None if the section
    cannot be found or if multiple sections match (ambiguous).
    """
